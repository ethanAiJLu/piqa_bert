...
def train(epochs,train_model_dir,train_mode,dev_mode,device="cuda",device_id=[0],train_model_start_step=0,batch_size=1,n_gpu=1):
    ...
    model = Model(hidden_size=512,dropout=0.5,bert_do_train=True).cuda(0)
    if(n_gpu>1):
    ...
    for epoch_idx in range(epochs):
        for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
            ...
            time1=time.time()
            model_output = model(context_idx,context_mask,question_idx,question_mask,c_ids,question_ids)
            ...
            if (steps % 1000 == 0):
                ...
                with torch.no_grad():
                    ...
                    for dev_step, dev_batch in enumerate(tqdm(dev_loader, desc="Iteration")):
                        dev_input_ids, dev_input_mask, dev_segment_ids, dev_start_positions, dev_end_positions = dev_batch["tensor_data"]
                        c_ids = dev_batch["c_ids"]
                        question_ids = dev_batch["question_id"]
                        dev_context_idx, dev_context_mask, dev_question_idx, dev_question_mask = handle_batch(
                            dev_input_ids)
                        dev_model_output = model(dev_context_idx, dev_context_mask, dev_question_idx, dev_question_mask,c_ids,question_ids)
